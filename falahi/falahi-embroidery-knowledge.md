# Falahi (فلاحي) — Palestinian/Levantine tatreez cross-stitch garment embroidery

> **Status: REGISTERED + INGESTED** (2026-07-12) — `falahi` is a `SUPPORTED_CATEGORIES` class;
> **11 ground-truth pairs** live in `falahi/pairs/falahi-01..11/` and feed
...
Falahi is generated by a dedicated **counted cross-stitch** primitive, not the run/satin/tatami
tiers. It is selected by `--cross-stitch` (AUTO-on for `config.CROSS_STITCH_CATEGORIES` = falahi;
...
- **Grid.** A fixed square grid over the palette-quantised image; cell pitch = the pair prior
  (falahi's satin-width median ≈ 2.1 mm) or `--cross-stitch-pitch-mm`. Each cell takes the
...
- **Verified**: a rasterised falahi-05 run scores in-band on satin_frac (100), fill_frac (0),
...
6. Add falahi to the router docs (`EMBROIDERY-PLAYBOOK.md`, orchestrator routing) so a dropped
